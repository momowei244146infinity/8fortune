...
class QuizDisplay:
    ...
    def __init__(self):
        ...
        if st.button("Reset"):
            self.get_random_list.clear()
        
        random_lst = self.get_random_list()
        #st.write(random_lst)
    ...
    @st.dialog("Lets start pillar meloday quiz", width='large')
    def display_question_main1(self):
        
        if 'current_pillar' not in st.session_state:
            
            # use reset method
            pillar_random = self.get_random_list()#random.sample(list(pillar), len(pillar))
        ...
        if st.button("Next Question", key=f"melody_select_next_{st.session_state['pillar_melody_cnt']}", disabled=not st.session_state['answered']):
            # Get next question\
            self.question1_next()
    ...
    def get_random_list(_self):
        pillar_random = random.sample(list(pillar), len(pillar))
        return pillar_random
           
    def question1(self):
        index = st.session_state['pillar_melody_cnt']
        pillar_item = self.get_random_list()[index]
        ...
        # get rid of user answer in session state
    ...
    def question1_next(self):
        st.session_state['answered'] = False
        if st.session_state['pillar_melody_cnt'] < len(self.get_random_list()):
            #index = st.session_state['pillar_melody_cnt']
            #current_pillar = self.get_random_list()[index]
            #st.session_state.answered = False
            #st.session_state['pillar_melody_user_answer'] = ""
    ...
    def display_question_main3(self):
        ...
        #display question
        # get userinput
        user_answer, current_period_pillar1, current_period_pillar2 = self.question3()
        ...
        # review
        if st.button("Rview Round answers"):
